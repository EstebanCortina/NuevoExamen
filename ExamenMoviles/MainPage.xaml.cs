...
using SQLite;
using ExamenMoviles.Pages;
using ExamenMoviles.Resources.Splash;
...
public partial class MainPage : ContentPage
{
    ...
    async private void eventoTapConfig()
    {
        configImg.GestureRecognizers.Add(new TapGestureRecognizer
        {
    ...
    async private void CambiarSplashScreen()
    {
        if (!Preferences.Get("statusSplash", false))
        {
            try
            {
                await Navigation.PushAsync(new CustomSplash());
            }
            catch (Exception ex)
            {
        ...
        {
            Preferences.Set("statusSplash", false);
        }

    }

    private void ValidarNombreTienda()
    {
        lblTiendaName.Text += "a "+Preferences.Get("nombreTienda", "Tienda");
    }
